...
Mozkey 5.11 / Mozc 3.34 / *2026-09-15*
------------------------------------------

### 5.11 の変更内容

- 「Zenz 補正を直接表示」の待機時間の既定値を 96 ms から 150 ms に変更しました。既存プロファイルに保存済みの値は変更しません。
- 「Zenz 補正を直接表示」をライブ変換に対応しました。また、直接表示時に Mozc の中間結果を隠して待つ時間を設定できるようにしました。
- 局所補正の適用精度と学習方法を改善しました。
- 局所補正を「変換の好み」として手動で登録できるようにしました。

User-facing release version: `5.11`
Mozc internal version: `3.34.6231.100`

Mozkey 5.10 / Mozc 3.34 / *2026-09-15*
------------------------------------------
...
Windows版MSIはコード署名されていないため、環境によってはWindows
Defender / SmartScreenに警告またはブロックされる場合があります。

発行元を確認できないという警告だけが表示された場合は、「詳細情報」→
「実行」または「実行する」を選択してインストールを続行してください。
マルウェア警告が表示された場合は実行しないでください。

### macOSへのインストール（重要）

macOS版PKGはDeveloper ID署名・公証を行っていないため、macOSにインストールを
止められる場合があります。

その場合は、「システム設定」→「プライバシーとセキュリティ」を開き、
ブロックされたインストーラーの「このまま開く」（または「Open Anyway」）を
選んで明示的に承認してください。承認後、PKGをもう一度開きます。この承認を
行わない限り、macOS版はインストールできません。

User-facing release version: `5.10`
Mozc internal version: `3.34.6230.100`

Mozkey 4.89.3 / Mozc 3.34 / *2026-09-13*
------------------------------------------

### 4.89.3 の変更内容

- Local補正とMozcの長いユーザー履歴表記が一部だけ重なる場合に、Localと無関係な表記差まで履歴保護の例外になる問題を修正しました。
- 逆変換の文節が粗い場合でも、読みを再検証できた範囲ではLocal自動学習を語彙境界まで縮めます。たとえば「計量化の→軽量化の」ではなく「計量化→軽量化」として再利用しやすく学習します。
- 複数Localの適用後フィードバックについて、一方だけを戻した場合に他方を中立のまま保持する回帰テストを追加しました。

User-facing release version: `4.89.3`
Mozc internal version: `3.34.6228.100`

Mozkey 4.89.2 / Mozc 3.34 / *2026-09-13*
------------------------------------------

### 4.89.2 の変更内容

Local補正の適用判定を、全文逆変換の文節境界ではなく、ZenzとMozcのUnicode文字列アラインメントに変更しました。登録済みLocalだけを安全に適用しつつ、未登録のZenz補正を保持し、独立した複数Localを同時に扱えるようにしました。

User-facing release version: `4.89.2`
Mozc internal version: `3.34.6227.100`

Mozkey 4.89.1 / Mozc 3.34 / *2026-09-13*
------------------------------------------

### 4.89.1 の変更内容

「変換の好み」に登録したローカル補正が、Zenz と Mozc の全文逆変換で
文節境界を確定できない場合にも、現在の Mozc 候補と完全に一致する範囲で
適用されるようにしました。たとえば「計量化→軽量化」を登録している場合、
「けいりょうかのけんとう」に対する Zenz の「計量化の検討」を
「軽量化の検討」へ補正できます。

### 配布ファイル

- Windows: x64版MSI。
- macOS: Apple Silicon・Intel Mac対応のUniversal Zenz PKG。

User-facing release version: `4.89.1`
Mozc internal version: `3.34.6226.100`

Mozkey 4.89 / Mozc 3.34 / *2026-09-11*
----------------------------------------

### 4.89 の変更内容

4.87のリリース内容を維持したまま、Zenz補正の最終状態処理を安定化しました。

- 「変換の好み」で、読み・Zenz表記・希望する表記を指定できます。たとえば「よい→良い」のように、よく使う変換を学習回数を待たずに登録できます。
- 「Zenz 補正を直接表示」を使うと、Zenzの結果が早く返った場合は補正後の文字を直接表示し、時間がかかる場合はMozcの結果を表示します。入力中の表示を保ちながら、後から届いたZenz結果も正しく反映します。
- 直接表示を使っている場合でも、Mozcの変換状態やZenzの処理世代が途中で別のものに置き換わらないようにしました。入力の追加、Enter、Backspace、Deleteなどの操作が意図した順序で処理されます。
- Zenzの補正結果を採用したか、通常のMozc候補に戻したかを学習できます。Zenz学習データの管理画面では、全文の学習結果、自動学習、手動で指定した変換の好みをまとめて確認できます。
...
User-facing release version: `4.89`
Mozc internal version: `3.34.6225.100`

Mozkey 4.88 / Mozc 3.34 / *2026-09-11*
----------------------------------------
...
User-facing release version: `4.88`
Mozc internal version: `3.34.6224.100`

Mozkey 4.87 / Mozc 3.34 / *2026-09-11*
----------------------------------------

### 4.87 の変更内容

4.86のリリース内容を維持したまま、Zenz補正の開始タイミングと直接表示の扱いを整理しました。
...
User-facing release version: `4.87`
Mozc internal version: `3.34.6223.100`

Mozkey 4.86 / Mozc 3.34 / *2026-09-10*
----------------------------------------

### 4.86 の変更内容

4.84以降に追加されたZenz補正と変換設定を、より安定して使えるように整理しました。
...
User-facing release version: `4.86`
Mozc internal version: `3.34.6222.100`

Mozkey 4.85.1 / Mozc 3.34 / *2026-09-10*
-------------------------------------------

### 4.85.1 の変更内容

4.84以降、Zenz補正をより自然に使えるよう、変換結果の表示と学習の仕組みを強化しました。

- 「Zenz 補正を直接表示」を有効にすると、Zenzの結果が速く得られた場合は補正後の文字を直接表示します。処理に時間がかかる場合はMozcの結果を表示するため、入力の反応を保ちやすくなります。
- Zenzの候補を採用したか、通常の候補に戻したかを学習できるようになりました。使うほど、よく選ぶ変換を反映しやすくなります。
- 「変換の好み」では、読み・Zenz表記・好みの表記を指定できます。たとえば、特定の読みをいつも希望する漢字表記にしたい場合に、学習回数を待たずに登録できます。
- Zenz学習データの管理画面から、全文の学習結果、局所表記の自動学習、手動で指定した変換の好みをまとめて確認できます。手動設定を追加・削除しても、自動学習の記録はそのまま保持されます。
...
User-facing release version: `4.85.1`
...
遅延設定の説明も見直し、直接表示が有効な場合は使用されないことが分かるようにしました。
...
「Zenz 補正を直接表示」をライブ変換でも使えるようになりました。速く結果が出たときは、途中の変換結果を表示せず、補正後の文字を直接表示します。
Zenzの処理に時間がかかる場合は、Mozcの結果をすぐに表示します。入力の反応が遅くなりにくい動作です。
この機能は初期状態ではOFFです。使う場合は設定画面で「Zenz 補正を直接表示」をONにしてください。
Spaceキーで行う通常の変換も、これまでどおり利用できます。

バックスペースやDeleteで文字を削除したときも、変換中の表示が一度ひらがなに戻らず、現在の変換結果をすぐに表示します。

設定画面を開き直したりライブ変換を切り替えたりしても、「Zenz 補正を直接表示」の状態に応じて遅延設定が正しく無効化されます。

Windows x64 MSIとmacOS Universal Zenz PKGはGitHub Actionsでビルド・検証します。

User-facing release version: `4.82.0`
Mozc internal version: `3.34.6218.100`

Mozkey 4.80.4 / Mozc 3.34 / *2026-09-09*
-------------------------------------------
...
User-facing release version: `4.80.4`
...
設定画面で「Zenz 補正を直接表示（遅い場合は Mozc を使用）」と、Zenzの条件設定が重なって表示される問題を修正しました。各設定項目が正しい位置に表示されます。

### 4.80.1 の変更内容

4.80.0のZenz直接表示で、表示猶予時間を過ぎた後にすでにキューへ入っていた結果を誤って採用する可能性がある問題を修正しました。また、Windows版の更新時に新しい実行ファイルへ置き換わらないことがある問題と、「バージョン情報」に内部Mozcバージョンが表示される問題を修正しました。

### 4.80.0 の変更内容

Zenzの補正結果を、十分に速い場合はMozcの途中結果を表示せず直接表示できるオプションを追加しました。デフォルトはOFFで、OFFの場合は4.70.0と同じ動作です。Spaceによる通常変換だけが対象で、ライブ変換は変更していません。
...
User-facing release version: `4.80.2`
Mozc internal version: `3.34.6216.100`

Mozkey 4.80.1 / Mozc 3.34 / *2026-09-08*
-------------------------------------------

### 4.80.1 の変更内容

4.80.0のZenz直接表示で、表示猶予時間を過ぎた後にすでにキューへ入っていた結果を誤って採用する可能性がある問題を修正しました。

- 表示猶予の期限を結果の取得より先に判定し、期限後に到着したZenz結果を画面表示・Full/Local学習の対象から除外します。
- 期限内に取得できたZenz結果と、期限切れ時に表示するMozc結果の順序を回帰テストで確認します。
...
User-facing release version: `4.80.1`
Mozc internal version: `3.34.6215.100`

Mozkey 4.80.0 / Mozc 3.34 / *2026-09-08*
-------------------------------------------

### 4.80 の変更内容

Zenzの補正結果を、十分に速い場合はMozcの途中結果を表示せず直接表示できるオプションを追加しました。

- 設定画面の「Zenz 補正を直接表示（遅い場合は Mozc を使用）」で有効化できます。デフォルトはOFFで、OFFの場合は4.70.0と同じ動作です。
- Spaceによる通常変換だけが対象で、ライブ変換の動作は変更していません。
- Zenz対象外・非採用・タイムアウト時はMozcを表示します。表示保留中はSpace直前のpreeditをそのまま保持し、候補窓を一時的に隠します。
- 現行の表示保留時間は`min(Zenz timeout + 2 × poll interval, 250 ms)`で、デフォルト設定では228msです。遅れて到着したZenz結果は表示せず、学習にも使用しません。
...
User-facing release version: `4.80.0`
Mozc internal version: `3.34.6215.100`
...
Windowsの秀丸など、左右文脈を取得できないアプリで、変換をEnterで確定した直後の文字列を次のZenz補正の左文脈として継続できるようにしました。Wordのように確定直後に古い文脈を返すアプリでも、確定済みの文字列を失わないようにしています。Mac Wordのように文脈取得を抑制するアプリでは、同じSession側の揮発性継続文脈を使用します。

- Input.contextが省略された場合や、確定前の文脈が再送された場合も、直前の確定文字列を継続文脈に反映します。
- Shiftなどの単独の修飾キーでは継続状態を保ち、カーソル移動などの入力では破棄します。
- カーソル位置を移動する部分確定や、パスワード入力、フォーカス変更・明示的なリセットでは継続文脈を使用しません。
- Windows/macOSのリリースビルドは、Visual Studio・CRT・Bazelの環境差を検証してからインストーラーを生成します。
...
インストールを拒否された場合は、「システム設定 > プライバシーとセキュリティ」を開き、ブロックされたインストーラーに表示される「このまま開く」（または同等の「Open Anyway」）を明示的に選択してから、PKGをもう一度開いてください。「プライバシーとセキュリティ」でこの許可を明示的に与えない限り、macOS版はインストールできません。

macOS版のファイル名は `mozkey-space_4.60_macos_universal_zenz.pkg` です。SHA-256チェックサムは、同じリリースに添付します。

User-facing release version: `4.60.4`
Mozc internal version: `3.34.6215.100`

Mozkey 4.40.0 / Mozc 3.34 / *2026-09-04*
-------------------------------------------

### 4.40 の変更内容

ZenzのLocal学習を、長い文章の中でも活用しやすく改善しました。

- 既に成熟したLocalルール（例: `りせき / 離籍 → 離席`）を、「すみませんりせきします」のような長い文章の中でも、対応する局所部分に適用できるようにしました。
- まず読みと表記の一意な対応付けを確認し、対応付けが難しい場合も、表層差分から推定した局所ルールが既存の成熟ルールと完全に一致するときだけ採用します。
- 同じ読みが繰り返される文章や、複数通りに解釈できる文章では、誤った補正や学習を避けるため保守的に適用しません。
- 自動適用の処理で新しい学習実績を水増しせず、適用済みLocalルールの記録も維持します。
- Full Feedbackの意味論、現在のMozc変換結果によるLocal候補の選択、表示中のZenz候補だけを対象にする安全確認は変更していません。
...
macOS版のファイル名は `mozkey-space_4.40_macos_universal_zenz.pkg` です。SHA-256チェックサムは、同じリリースに添付します。

User-facing release version: `4.40.0`
Mozc internal version: `3.34.6210.100`

Mozkey 4.30.0 / Mozc 3.34 / *2026-09-04*
-------------------------------------------

### 4.30 の追加内容

Zenzフィードバック学習の Local canonical identity（ローカル学習の識別方法）と保存処理を改善しました。ユーザーの修正を一意に対応付けられる場合は、安全に局所化した最小の読み、Zenzが最初に出した表記、修正後の表記を組み合わせて学習します。学習した局所ルールは、現在のMozc変換結果が修正後の表記を独立に支持し、読みとの対応を一意に確認できた場合だけ使用します。

同じ読みが文章中に複数回現れる場合も、現在のMozc変換結果に応じて各出現箇所を正しく補正・学習できるようにしました。Zenz却下後の通常Mozc確定経路、部分確定、競合時のフィードバック保存、途中で切れたTSVの復旧も強化しています。macOSでは内部TSVの一時ファイルを内容の書き込み前から所有者だけが読める`0600`にして保護します。Local threshold UIの上限表示も実装に合わせて整えています。Full Feedbackの意味論と、表示中のZenz候補だけを対象にする安全確認は変更していません。

### Zenzの学習データについて（重要）

このリリースでは、Zenzの学習データがリセットされます。既存のZenz学習データは新しいLocal canonical形式へ移行せず、クリアされた状態から再学習が始まります。通常のMozcユーザー辞書と変換履歴はリセットされません。
...
macOS版のファイル名は `mozkey-space_4.30_macos_universal_zenz.pkg` です。SHA-256チェックサムは、同じリリースに添付しています。

User-facing release version: `4.30.0`
Mozc internal version: `3.34.6210.100`
...
macOS版のファイル名は `mozkey-space_4.10_macos_universal_zenz.pkg` です。SHA-256チェックサムは、同じリリースに添付しています。
