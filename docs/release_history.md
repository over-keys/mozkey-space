...
Mozkey 4.20.0 / Mozc 3.34 / *2026-09-04*
-------------------------------------------
...
同じ読みが文章中に複数回現れる場合の補正・学習、Zenz却下後の通常Mozc確定経路、部分確定、競合時のフィードバック保存、途中で切れたTSVの復旧を強化しました。内部TSVと一時ファイルの保護、Local threshold UIの上限表示も実装に合わせて整えています。Full Feedbackの意味論と、表示中のZenz候補だけを対象にする安全確認は変更していません。

### Zenzの学習データについて（重要）

このリリースでは、Zenzの学習データがリセットされます。既存のZenz学習データは新しいLocal canonical形式へ移行せず、クリアされた状態から再学習が始まります。通常のMozcユーザー辞書と変換履歴はリセットされません。

User-facing release version: `4.20.0`
