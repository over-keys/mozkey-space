...
- 4.85.0で「変換の好み」が表示されない問題を修正しました。設定の「入力補助」→「Zenz 学習データ」の「管理...」→「変換の好み...」から、読み・Zenz表記・好みの表記を登録・削除できます。
- 管理画面を統一し、全文の学習結果、局所表記の自動学習、手動の変換の好みを同じ入口から確認できるようにしました。手動設定の追加・削除で、自動学習の回数は変わりません。
- 手動設定を登録すると、安全に対応できる変換で学習回数に関係なく利用されます。局所学習が無効のときは、登録しても変換には使われないことを画面に表示します。
- Windowsでは、Zenzとの通信が止まったときに処理を打ち切る仕組みを修正しました。
